<template>
  <div id="mapTrack">
    <el-card class="box-card" shadow="never" :body-style="{ padding: '0' }">
      <div
        id="map-test"
        :style="{ height: 'calc(100vh - ' + (isShowDetail ? 297 : 57) + 'px)' }"
      >
        <div id="loadingTip" v-if="beforeInit && initSimplifier">
          加载数据，请稍候...
        </div>
      </div>
      ...
      <div class="guijiset form-table-search">
        <div class="rowset">
          <label>机械号码：</label><label>{{ vehicleNo }}</label>
        </div>
        <div class="rowset">
          <label>开始时间：</label>
          <el-date-picker
...
export default {
  ...
  methods: {
    ...
    loadMore() {
      ...
      let trackTableData = this.trackTableDataBak.slice(
        (this.pageNo - 1) * this.pageSize,
        this.pageSize + (this.pageNo - 1) * this.pageSize
      );
        ...
        if (linArray.length > 0) {
          ...
          that.navgtr.on("move", function(data, position) {
            ...
            let content = [
              ...
              "<div>速&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;度 : " +
                pointDataList[idx].cs +
                "(km/h)</div>",
              "<div>经&nbsp;&nbsp;纬&nbsp;&nbsp;度 : " +
                pointDataList[idx].lng +
                "," +
                pointDataList[idx].lat +
                "</div>",
              "</div>"
            ];
    ...
    sliderChange(val) {
      ...
      let decimal =
        String((newVal / 100) * this.actualList.length).split(".")[1] || 0;
    ...
    queryPointInfo(row, column) {
      ...
      let position = [
        this.trackData[row.index].lng,
        this.trackData[row.index].lat
      ];
