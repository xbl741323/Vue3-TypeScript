...
export const pageRoutes = [
  ...
  { path: '/bigData', component: () => import('@/views/pages/bigData'), name: 'bigData', meta: { title: '大数据' } }
...
export const moduleRoutes = {
  ...
  children: [
    ...
    { path: '/iframe', component: null, name: 'iframe', meta: { title: 'iframe', isTab: true } },
      {
          path: "/pagecomponentconfigtree",
          name: "pagecomponentconfigtree",
          component: () =>
              import("@/views/modules/monitorconfig/pagecomponentconfigtree.vue"),
          meta: { title: "组件详细配置", isTab: true },
      }
  ]
}
...
function fnAddDynamicMenuRoutes (menuList = [], routes = []) {
  ...
  for (var i = 0; i < menuList.length; i++) {
    ...
    if(menuList[i].url.indexOf("/:") !=-1 ){
      let menuUrl = menuList[i].url.split('/:');
      menuList[i].url = menuUrl[0];
      let querys = {};
      for (var iUrl=1;iUrl<menuUrl.length;iUrl++){
        let keys = menuUrl[iUrl].split('=');
        querys[keys[0]]=keys[1];
      }
      route['query']=querys;
