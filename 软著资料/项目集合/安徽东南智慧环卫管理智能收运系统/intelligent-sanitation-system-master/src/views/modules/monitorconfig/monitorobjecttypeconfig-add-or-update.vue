<template>
  <el-dialog :visible.sync="visible" :title="!dataForm.id ? $t('add') : $t('update')" :close-on-click-modal="false" :close-on-press-escape="false">
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmitHandle()" :label-width="$i18n.locale === 'en-US' ? '120px' : '80px'">
            ...
            <el-form-item label="监控对象大类code" prop="objectBigTypeCode">
              <el-input v-model="dataForm.objectBigTypeCode" placeholder="监控对象大类code"></el-input>
            </el-form-item>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 获取信息
      getInfo () {
        this.$http.get('/monitorconfig/monitorobjecttypeconfig/' + this.dataForm.id).then(({ data: res }) => {
          if (res.code !== 0) {
      ...
      dataFormSubmitHandle: debounce(function () {
        this.$refs['dataForm'].validate((valid) => {
          ...
          this.$http[!this.dataForm.id ? 'post' : 'put']('/monitorconfig/monitorobjecttypeconfig/', this.dataForm).then(({ data: res }) => {
            if (res.code !== 0) {
              return this.$message.error(res.msg)
            }
            this.$message({
