<template>
  <el-card shadow="never" class="aui-card--fill">
    <div class="mod-worktarget__twgtdustbin">
      <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
        ...
        <el-form-item>
          <el-button v-if="$hasPermission('worktarget:twgtdustbin:save')" type="primary" @click="addOrUpdateHandle()">{{ $t('add') }}</el-button>
        </el-form-item>
        <el-form-item>
          <el-button v-if="$hasPermission('worktarget:twgtdustbin:delete')" type="danger" @click="deleteHandle()">{{ $t('deleteBatch') }}</el-button>
        </el-form-item>
      </el-form>
      <div class="dustbin-table-container" :style="[{ width: foldStatus ? '60%' : '0px' }]">
        <el-table v-loading="dataListLoading" :data="dataList" border @selection-change="dataListSelectionChangeHandle" style="width: 100%;" @row-click="onRowClick" @select="onRowSelect">
          ...
          <el-table-column prop="ifidCode" label="IFID" header-align="center" align="center"></el-table-column>
          <el-table-column prop="capacity" label="容积" header-align="center" align="center"></el-table-column>
          <el-table-column prop="dustbinTypeName" label="垃圾类型" header-align="center" align="center"></el-table-column>
          <el-table-column prop="layToWorktargetName" label="安放位置" header-align="center" align="center"></el-table-column>
          <el-table-column :label="$t('handle')" fixed="right" header-align="center" align="center" width="150">
            <template slot-scope="scope">
              <el-button v-if="$hasPermission('worktarget:twgtdustbin:update')" type="text" size="small" @click="addOrUpdateHandle(scope.row.id)">{{ $t('update') }}</el-button>
              <el-button v-if="$hasPermission('worktarget:twgtdustbin:delete')" type="text" size="small" @click="deleteHandle(scope.row.id)">{{ $t('delete') }}</el-button>
            </template>
          </el-table-column>
        </el-table>
        <el-pagination
      ...
      <div class="dustbin-map" id="dustbin-container" :style="[{ width: foldStatus ? '40%' : '100%' }]">
      </div>
      <!-- 弹窗, 新增 / 修改 -->
...
import AddOrUpdate from './twgtdustbin-add-or-update'
...
export default {
  ...
  data () {
    ...
    return {
      ...
      mixinViewModuleOptions: {
        getDataListURL: '/worktarget/twgtdustbin/listPage',
        getDataListIsPage: true,
        exportURL: '/worktarget/twgtdustbin/export',
        deleteURL: '/worktarget/twgtdustbin',
        deleteIsBatch: true,
        createdIsNeed: false,
        ...
        for (var i = 0; i < this.markerList.length; i++) {
          ...
          if (markerObj.lng && markerObj.lat) {
            var marker = new AMap.Marker({
              ...
              icon: new AMap.Icon({ image: dustbinEdited,
  ...
  computed: {
    markerImg () {
      return (marker) => {
        return this.activeId == marker.id ? dustbinEditing : dustbinEdited
      }
    }
  },
  mounted () {
  ...
  methods: {
    initMap () {
      var amap = new AMap.Map('dustbin-container', {
        ...this.mapConfig
      })
      this.amap = amap
    },
    ...
    unActiveShow () {
      if (this.activeMarker) {
        this.activeMarker.setIcon(new AMap.Icon({ image: dustbinEdited,
          size: new AMap.Size(30, 30), // 设置icon的大小
          imageSize: new AMap.Size(20, 20) }))
      }
    },
    activeShow (markerId) {
      ...
      if (marker != null) {
        marker.setIcon(new AMap.Icon({ image: dustbinEditing,
          size: new AMap.Size(30, 30), // 设置icon的大小
          imageSize: new AMap.Size(20, 20) }))
