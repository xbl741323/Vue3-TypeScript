<template>
  <el-dialog :visible.sync="visible" width="80%" :title="!dataForm.id ? $t('add') : $t('update')" :close-on-click-modal="false" :close-on-press-escape="false" @close="onDialogClose">
          ...
          <el-row>
              <el-col :span="12">
                <el-form-item label="产出量" prop="output">
                  <el-input v-model="dataForm.output" oninput="value=value.replace(/[^0-9.]/g,'')" placeholder="产出量"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="12">
    ...
    <div class="shop-form-map">
          ...
          <div id="shop-edit-container" class="shop-edit-map">
          </div>
        </div>
    </div>
  </el-dialog>
...
export default {
  ...
  computed: {
    ...
    markerImg () {
      return () => {
        return this.dragging ? shopEditing : shopEdited
      }
    },
    showMarker () {
  ...
  methods: {
    initMap () {
      var editAmap = new AMap.Map('shop-edit-container', {
        ...this.mapConfig
      })
      editAmap.on('click', (e) => {
    ...
    // 获取信息
    getInfo () {
      this.$http.get('/worktarget/twgtshop/' + this.dataForm.id).then(({ data: res }) => {
        if (res.code !== 0) {
    ...
    dataFormSubmitHandle: debounce(function () {
      this.$refs['dataForm'].validate((valid) => {
        ...
        this.$http[!this.dataForm.id ? 'post' : 'put']('/worktarget/twgtshop/', this.dataForm).then(({ data: res }) => {
          if (res.code !== 0) {
            return this.$message.error(res.msg)
          }
          this.$message({
