<template>
  <el-dialog :visible.sync="visible" width="80%" :title="!dataForm.id ? $t('add') : $t('update')" :close-on-click-modal="false" :close-on-press-escape="false" @close="onDialogClose">
    <div class="institution-edit-mian">
      <div class="institution-form-container">
        <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmitHandle()" :label-width="$i18n.locale == 'en-US' ? '80px' : '80px'">
              ...
              <el-row>
                ...
                <el-col :span="12">
                  <el-form-item label="清运方式" prop="cleanType">
                    <dict-select :dictType="'cleanType'" v-model="dataForm.cleanType" @change="changeCleanType"></dict-select>
                  </el-form-item>
                </el-col>
              </el-row>
              <el-row>
      ...
      <div class="institution-form-map">
        <div id="institution-edit-container" class="institution-edit-map">
          <div class="text-btn">
...
export default {
  ...
  computed: {
    ...
    markerImg () {
      return () => {
        return this.dragging ? institutionEditing : institutionEdited
      }
    },
    showMarker () {
  ...
  methods: {
    initMap () {
      var editAmap = new AMap.Map('institution-edit-container', {
        ...this.mapConfig
      })
      editAmap.on('click', (e) => {
    ...
    // 获取信息
    getInfo () {
      this.$http.get('/worktarget/twgtinstitution/' + this.dataForm.id).then(({ data: res }) => {
        if (res.code !== 0) {
    ...
    openDraw () {
      ...
      if (this.marker) {
        ...
        this.marker.setIcon(institutionEditing)
      }
    },
    closeDraw () {
      ...
      this.marker.setIcon(institutionEdited)
      this.dragging = false
    },
    onSearchResult (pois) {
      if (pois.length > 0) {
    ...
    createMarker (lng, lat) {
      var marker = new AMap.Marker({
        icon: this.dragging ? institutionEditing : institutionEdited,
        position: [lng, lat],
    ...
    dataFormSubmitHandle: debounce(function () {
      this.$refs['dataForm'].validate((valid) => {
        ...
        this.$http[!this.dataForm.id ? 'post' : 'put']('/worktarget/twgtinstitution/', this.dataForm).then(({ data: res }) => {
          if (res.code !== 0) {
            return this.$message.error(res.msg)
          }
          this.$message({
