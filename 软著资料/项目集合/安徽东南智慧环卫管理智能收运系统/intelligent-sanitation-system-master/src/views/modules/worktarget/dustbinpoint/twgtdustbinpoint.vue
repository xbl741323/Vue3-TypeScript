<template>
  <el-card shadow="never" class="aui-card--fill">
    <div class="mod-worktarget__twgtdustbinpoint">
      <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
        ...
        <el-form-item>
          <el-button v-if="$hasPermission('worktarget:twgtdustbinpoint:save')" type="primary" @click="addOrUpdateHandle()">{{ $t('add') }}</el-button>
        </el-form-item>
        <el-form-item>
          <el-button v-if="$hasPermission('worktarget:twgtdustbinpoint:delete')" type="danger" @click="deleteHandle()">{{ $t('deleteBatch') }}</el-button>
        </el-form-item>
      </el-form>
       <div class="table-container" :style="[{ width: foldStatus ? '60%' : '0px' }]">
        ...
        <el-table-column :label="$t('handle')" fixed="right" header-align="center" align="center" width="150">
          <template slot-scope="scope">
            <el-button v-if="$hasPermission('worktarget:twgtdustbinpoint:update')" type="text" size="small" @click="addOrUpdateHandle(scope.row.id)">{{ $t('update') }}</el-button>
            <el-button v-if="$hasPermission('worktarget:twgtdustbinpoint:delete')" type="text" size="small" @click="deleteHandle(scope.row.id)">{{ $t('delete') }}</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
      ...
      <div class="map" id="dustbinpoint-container" :style="[{ width: foldStatus ? '40%' : '100%' }]">
      </div>
      <!-- 弹窗, 新增 / 修改 -->
...
import AddOrUpdate from './twgtdustbinpoint-add-or-update'
...
export default {
  ...
  data () {
    ...
    return {
      ...
      mixinViewModuleOptions: {
        getDataListURL: '/worktarget/twgtdustbinpoint/listPage',
        getDataListIsPage: true,
        exportURL: '/worktarget/twgtdustbinpoint/export',
        deleteURL: '/worktarget/twgtdustbinpoint',
        deleteIsBatch: true,
        createdIsNeed: false,
        ...
        for (var i = 0; i < this.markerList.length; i++) {
          ...
          if (markerObj.lng && markerObj.lat) {
            var marker = new AMap.Marker({
              ...
              icon: new AMap.Icon({ image: dustbinpointEdited,
  ...
  computed: {
    markerImg () {
      return (marker) => {
        return this.activeId == marker.id ? dustbinpointEditing : dustbinpointEdited
      }
    }
  },
  mounted () {
  ...
  methods: {
    initMap () {
      var amap = new AMap.Map('dustbinpoint-container', {
        ...this.mapConfig
      })
      this.amap = amap
    },
    ...
    unActiveShow () {
      if (this.activeMarker) {
        this.activeMarker.setIcon(new AMap.Icon({ image: dustbinpointEdited,
          size: new AMap.Size(30, 30), // 设置icon的大小
          imageSize: new AMap.Size(20, 20) }))
      }
    },
    activeShow (markerId) {
      ...
      if (marker != null) {
        marker.setIcon(new AMap.Icon({ image: dustbinpointEditing,
          size: new AMap.Size(30, 30), // 设置icon的大小
          imageSize: new AMap.Size(20, 20) }))
