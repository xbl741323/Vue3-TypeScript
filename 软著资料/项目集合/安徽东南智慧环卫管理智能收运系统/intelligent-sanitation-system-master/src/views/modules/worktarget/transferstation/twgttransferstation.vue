<template>
  <el-card shadow="never" class="aui-card--fill">
    <div class="mod-worktarget__twgttransferstation">
      <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
        ...
        <el-form-item>
          <el-button v-if="$hasPermission('worktarget:twgttransferstation:save')" type="primary" @click="addOrUpdateHandle()">{{ $t('add') }}</el-button>
        </el-form-item>
        <el-form-item>
          <el-button v-if="$hasPermission('worktarget:twgttransferstation:delete')" type="danger" @click="deleteHandle()">{{ $t('deleteBatch') }}</el-button>
        </el-form-item>
      </el-form>
      <div class="table-container" :style="[{ width: foldStatus ? '60%' : '0px' }]">
          <el-table v-loading="dataListLoading" :data="dataList" border @selection-change="dataListSelectionChangeHandle" style="width: 100%;" @row-click="onRowClick" >
            ...
            <el-table-column prop="capacity" label="吞吐量" header-align="center" align="center"></el-table-column>
            ...
            <el-table-column :label="$t('handle')" fixed="right" header-align="center" align="center" width="150">
              <template slot-scope="scope">
                <el-button v-if="$hasPermission('worktarget:twgttransferstation:update')" type="text" size="small" @click="addOrUpdateHandle(scope.row.id)">{{ $t('update') }}</el-button>
                <el-button v-if="$hasPermission('worktarget:twgttransferstation:delete')" type="text" size="small" @click="deleteHandle(scope.row.id)">{{ $t('delete') }}</el-button>
              </template>
            </el-table-column>
          </el-table>
          <el-pagination
      ...
      <!-- 地图 -->
        <div class="transferstation-map" id="transferstation-container" :style="[{ width: foldStatus ? '40%' : '100%' }]">
        </div>
      <!-- 弹窗, 新增 / 修改 -->
...
import AddOrUpdate from './twgttransferstation-add-or-update'
...
export default {
  ...
  data () {
    return {
      ...
      mixinViewModuleOptions: {
        getDataListURL: '/worktarget/twgttransferstation/listPage',
        getDataListIsPage: true,
        exportURL: '/worktarget/twgttransferstation/export',
        deleteURL: '/worktarget/twgttransferstation',
        deleteIsBatch: true,
        createdIsNeed: false,
  ...
  methods: {
    initMap () {
      var amap = new AMap.Map('transferstation-container', {
        ...this.mapConfig
      })
      this.amap = amap
    },
    ...
    openWindowInfo (polygon) {
      ...
      // info.push('<p>部门：' + polygon.orgName + '</p>')
      // info.push('<p>吞吐量：' + polygon.capacity + '</p>')
      ...
      info.push('<tr><td class="info-content-key">部门：</td><td class="info-content-value">' + polygon.orgName + '</td></tr>')
      info.push('<tr><td class="info-content-key">产出量：</td><td class="info-content-value">' + polygon.output + '</td></tr>')
