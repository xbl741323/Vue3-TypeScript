  ...
  <el-card shadow="never" class="aui-card--fill">
    <div class="mod-worktarget__twgtshop">
      <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
        ...
        <el-form-item>
          <el-button v-if="$hasPermission('worktarget:twgtshop:save')" type="primary" @click="addOrUpdateHandle()">{{ $t('add') }}</el-button>
        </el-form-item>
        <el-form-item>
          <el-button v-if="$hasPermission('worktarget:twgtshop:delete')" type="danger" @click="deleteHandle()">{{ $t('deleteBatch') }}</el-button>
        </el-form-item>
      </el-form>
        ...
        <el-table v-loading="dataListLoading" :data="dataList" border @selection-change="dataListSelectionChangeHandle" style="width: 100%;" @row-click="onRowClick" @select="onRowSelect">
          ...
          <el-table-column :label="$t('handle')" fixed="right" header-align="center" align="center" width="150">
            <template slot-scope="scope">
              <el-button v-if="$hasPermission('worktarget:twgtshop:update')" type="text" size="small" @click="addOrUpdateHandle(scope.row.id)">{{ $t('update') }}</el-button>
              <el-button v-if="$hasPermission('worktarget:twgtshop:delete')" type="text" size="small" @click="deleteHandle(scope.row.id)">{{ $t('delete') }}</el-button>
            </template>
          </el-table-column>
        </el-table>
        <el-pagination
      ...
      <!-- 地图 -->
      <div class="map" id="shop-container" :style="[{ width: foldStatus ? '40%' : '100%' }]">
      </div>
      <!-- 弹窗, 新增 / 修改 -->
...
import AddOrUpdate from './twgtshop-add-or-update'
...
export default {
  ...
  data () {
    ...
    return {
      ...
      mixinViewModuleOptions: {
        getDataListURL: '/worktarget/twgtshop/listPage',
        getDataListIsPage: true,
        exportURL: '/worktarget/twgtshop/export',
        deleteURL: '/worktarget/twgtshop',
        deleteIsBatch: true,
        createdIsNeed: false,
  ...
  methods: {
    initMap () {
      var amap = new AMap.Map('shop-container', {
        ...this.mapConfig
      })
      this.amap = amap
    },
