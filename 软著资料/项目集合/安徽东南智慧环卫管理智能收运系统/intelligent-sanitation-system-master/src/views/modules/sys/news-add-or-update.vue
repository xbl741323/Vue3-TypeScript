  ...
  methods: {
    init () {
      ...
      this.$nextTick(() => {
        ...
        if (this.dataForm.id) {
            ...
            //window.tinymce.get('noticeid').setContent( '');

           // this.$refs['noticeid'].setContent( '');
    ...
    getInfo () {
      this.$http.get(`/sys/news/${this.dataForm.id}`).then(({ data: res }) => {
        ...
        //  alert("  文本编辑设置2"+this.dataForm.content);

          //window.tinymce.get('noticeid').setContent( this.dataForm.content);
         // this.$refs['noticeid'].setContent( this.dataForm.content);
